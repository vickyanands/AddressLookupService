...
import { createErrorResponseForInternalServerErrors, createErrorResponseForInvalidUserRequest, createSuccessfullResponse as createSuccessfullResponse } from "../util/common";
...
const AddressLooupService: AddressLooupServiceIF = {
    lookupAddress: async (address: string): Promise<APIGatewayProxyStructuredResultV2> => {
        try {
            const { gpsCooridnatesResonseObject } = await (GPSCoordinates.getGPSCoordinatesForAddress(address));
            if (gpsCooridnatesResonseObject?.features?.length > 0) {
                ...
                const [responseDataStateElectoralDistrict, respDataForSuburbname] = await StateElectoralDistrictNameAndSuburb.fetchStateEleactoralDisctrictAndSuburbName(gpsCoordonates)
